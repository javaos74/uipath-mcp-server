...
async def get_or_create_mcp_server(tenant_name: str, server_name: str) -> DynamicMCPServer:
...
async def get_me(request):
    ...
    user_data = {k: v for k, v in user.model_dump().items() 
...
async def update_uipath_config(request):
    ...
    try:
        ...
        user_data = {k: v for k, v in updated_user.items() 
                     if k not in ["hashed_password", "uipath_access_token"]}
        user_response = UserResponse(**user_data)
...
async def sse_handler(request):
    ...
    # Check authentication (supports both server API token and JWT token)
    ...
    # Get or create SSE transport for this endpoint
    key = f"{tenant_name}/{server_name}"
    if key not in sse_transports:
        sse_transports[key] = SseServerTransport(f"/mcp/{tenant_name}/{server_name}/messages")
...
async def sse_message_handler(request):
    ...
    # Handle the POST message (this sends the response directly)
...
async def get_server_token(request):
    ...
    if token is None:
        return JSONResponse({
            "token": None,
            "message": "No API token generated yet"
        })
...
app = Starlette(
    ...
    routes=[
        ...
        # MCP SSE endpoint - format: /mcp/{tenant_name}/{server_name}
        Route("/mcp/{tenant_name}/{server_name}", sse_handler, methods=["GET"]),
        Route("/mcp/{tenant_name}/{server_name}/messages", sse_message_handler, methods=["POST"]),
        ...
        Route("/api/servers/{tenant_name}/{server_name}", update_server, methods=["PUT"]),
        Route("/api/servers/{tenant_name}/{server_name}", delete_server, methods=["DELETE"]),
        
        # MCP Server Token Management API
        Route("/api/servers/{tenant_name}/{server_name}/token", generate_server_token, methods=["POST"]),
        Route("/api/servers/{tenant_name}/{server_name}/token", get_server_token, methods=["GET"]),
        Route("/api/servers/{tenant_name}/{server_name}/token", revoke_server_token, methods=["DELETE"]),
        
        # MCP Tool Management API
        Route("/api/servers/{tenant_name}/{server_name}/tools", list_tools, methods=["GET"]),
        Route("/api/servers/{tenant_name}/{server_name}/tools", create_tool, methods=["POST"]),
        Route("/api/servers/{tenant_name}/{server_name}/tools/{tool_name}", get_tool, methods=["GET"]),
        Route("/api/servers/{tenant_name}/{server_name}/tools/{tool_name}", update_tool, methods=["PUT"]),
        Route("/api/servers/{tenant_name}/{server_name}/tools/{tool_name}", delete_tool, methods=["DELETE"]),
    ],
    on_startup=[startup],
)
